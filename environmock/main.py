...
class GameHandler(webapp2.RequestHandler):
    def get(self,token):
        ...
        htmldict = textadventure.htmlstr(user.token)
...
class RoomHandler(webapp2.RequestHandler):
    def get(self,token,room):
        self.response.write(htmldict[room])
...
class EditingHandler(webapp2.RequestHandler):
  ...
  Game:<br><textarea name="game">{game}</textarea><br>
  <input type="submit" value="Submit">
    ...
    def post(self,token):
        ...
        try:
            ...
            try:
                ...
                self.redirect(uri)
        htmldict = textadventure.htmlstr(token)
        query = User.query(User.token == token)
        user = query.get()
        user.game = game
        user.put()
